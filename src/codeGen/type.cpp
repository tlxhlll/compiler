...
namespace spc
{
    ...
    void RecordTypeDecl::append(const std::shared_ptr<VarDeclNode> &var)
    {
        for (auto &f : field)
        {
            if (f->name->name == var->name->name)
    ...
    void RecordTypeDecl::merge(const std::shared_ptr<RecordTypeDecl> &rhs)
    {
        for (auto &var : rhs->field)
        {
            field.push_back(var);
    ...
    void RecordTypeDecl::merge(std::shared_ptr<RecordTypeDecl> &&rhs)
    {
        field.merge(std::move(rhs->field));
        // Check duplicate
        std::set<std::string> nameSet;
    ...
    llvm::Type *RecordTypeDecl::getLLVMType(CodegenContext &context)
    { 
        std::vector<llvm::Type *> fieldTy;
        for (auto &decl: field)
        {
    ...
    void RecordTypeDecl::insertNestedRecord(const std::string &outer, CodegenContext &context)
    {
        for (auto &f : this->field)
        {
            auto fTy = f->type;
    ...
    llvm::Value *RecordTypeDecl::getFieldIdx(const std::string &name, CodegenContext &context)
    {
        unsigned i = 0;
        for (auto &f : field)
        {
