...
namespace spc
{
    llvm::Value *VarDeclNode::createGlobalArray(CodegenContext &context, const std::shared_ptr<ArrayTypeNode> &arrTy)
    {
        ...
        if (!endIdx)
            {std::cout<<"eout2"<<std::endl;throw CodegenException("End index invalid");}
        else if (start > end)
            {std::cout<<"eout2"<<std::endl;throw CodegenException("End index must be greater than start index!");}
        else if (endIdx->getBitWidth() <= 32)
            len = end - start + 1;
        else
            {std::cout<<"eout2"<<std::endl;throw CodegenException("End index overflow");}
    ...
    llvm::Value *VarDeclNode::createArray(CodegenContext &context, const std::shared_ptr<ArrayTypeNode> &arrTy)
    {
        ...
        else if (ty->isArrayTy())
        {
            ...
            {std::cout<<"eout2"<<std::endl;throw CodegenException("Unsupported type of array");}

        llvm::ConstantInt *startIdx = llvm::dyn_cast<llvm::ConstantInt>(arrTy->range_start->codegen(context));
        if (!startIdx)
            {std::cout<<"eout2"<<std::endl;throw CodegenException("Start index invalid");}
        ...
        if (!success) {std::cout<<"eout2"<<std::endl;throw CodegenException("Duplicate identifier in var section of function " + context.getTrace() + ": " + this->name->name);}
        context.log() << "\tCreated array " << this->name->name << std::endl;
    ...
    llvm::Value *VarDeclNode::codegen(CodegenContext &context)
    {
        if (context.is_subroutine)
        {
            ...
            else
            {
                ...
                if (!success) {std::cout<<"eout2"<<std::endl;throw CodegenException("Duplicate identifier in var section of function " + context.getTrace() + ": " + name->name);}
                return local;
            }
        }
        else
        {
            if (context.getModule()->getGlobalVariable(name->name) != nullptr)
                {std::cout<<"eout2"<<std::endl;throw CodegenException("Duplicate global variable: " + name->name);}
            if (type->type == Type::Alias)
            {
                std::string aliasName = cast_node<AliasTypeNode>(type)->name->name;
                context.log() << "\tSearching alias " << aliasName << std::endl;
            ...
            else
            {
                ...
                if (ty->isIntegerTy()) 
                ...
                else if (ty->isStructTy())
       	        {
                    ...
                    {std::cout<<"eout2"<<std::endl;throw CodegenException("Unknown type");}
    ...
    llvm::Value *ConstDeclNode::codegen(CodegenContext &context)
    {
        if (context.is_subroutine)
        {
            if (val->type == Type::String)
            {
                ...
                if (!success) {std::cout<<"eout2"<<std::endl;throw CodegenException("Duplicate identifier in const section of function " + context.getTrace() + ": " + name->name);}
        ...
        else
        {
            ...
            else
            {
                ...
                if (!success) {std::cout<<"eout2"<<std::endl;throw CodegenException("Duplicate identifier in const section of main program: " + name->name);}
                context.log() << "\tAdded to symbol table" << std::endl;
                return nullptr;
            } 
        }
    ...
    llvm::Value *TypeDeclNode::codegen(CodegenContext &context)
    {
        if (context.is_subroutine)
        {
            if (type->type == Type::Array)
            {
                ...
                if (!success) {std::cout<<"eout2"<<std::endl;throw CodegenException("Duplicate type alias in function " + context.getTrace() + ": " + name->name);}
        ...
        else
        {
            if (type->type == Type::Array)
            {
                ...
                if (!success) {std::cout<<"eout2"<<std::endl;throw CodegenException("Duplicate type alias in main program: " + name->name);}
