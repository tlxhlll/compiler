...
namespace spc
{
    ...
    llvm::Value *BinaryExprNode::codegen(CodegenContext &context)
    {
        ...
        else if (lexp->getType()->isIntegerTy(8) && rexp->getType()->isIntegerTy(8)) 
        {
            ...
            if (it != iCmpMap.end())
                ...
                {eout4();throw CodegenException("Invaild operator for CHAR type");}
        }
        else
            {eout4();throw CodegenException("Invaild operation between different types");}
    }
    // Specify more info of Procedure
    llvm::Value *CustomProcNode::codegen(CodegenContext &context)
    {
        auto *func = context.getModule()->getFunction(name->name);
        if (!func)
            {eout4();throw CodegenException("Function not found: " + name->name + "()");}
        size_t argCnt = 0;
        int index = 0;
        if (args != nullptr)
            argCnt = args->getChildren().size();
        if (func->arg_size() != argCnt)
            {eout4();throw CodegenException("Wrong number of arguments: " + name->name + "()");}
        ...
        if (args != nullptr)
            for (auto &arg : args->getChildren())
            {
                ...
                else if (argTy->isDoubleTy() && paramTy->isIntegerTy(32))
                {
                    ...
                    {eout4();throw CodegenException("Incompatible type in the " + std::to_string(index) + "th arg when calling " + name->name + "()");}
    ...
    llvm::Value *SysProcNode::codegen(CodegenContext &context)
    {
        if (name == SysFunc::Write || name == SysFunc::Writeln) {
            ...
            {
                if (this->args->getChildren().size() > 1)
                {
                    auto arg1 = this->args->getChildren().front();
                    // auto arg2 = this->args->getChildren().back();
                    auto *value = arg1->codegen(context);
                    ...
                    {
                        func_args.push_back(context.getBuilder().CreateGlobalStringPtr("%.1f"));
                        func_args.push_back(value);
                    }
                    else if (value->getType()->isArrayTy()) // String
                    {
                        ...
                        if (!a->getElementType()->isIntegerTy(8))
                            {eout4();throw CodegenException("Cannot print a non-char array");}
                        func_args.push_back(context.getBuilder().CreateGlobalStringPtr("%s"));
                        llvm::Value *valuePtr;
                        if (is_ptr_of<LeftExprNode>(arg1))
                        {
                            auto argId = cast_node<LeftExprNode>(arg1);
                            valuePtr = argId->getPtr(context);
                        }
                        else if (is_ptr_of<CustomProcNode>(arg1))
                            valuePtr = value;
                        else
                            assert(0);
                    ...
                    else if (value->getType()->isPointerTy()) // String
                    {
                        ...
                        {eout4();throw CodegenException("Incompatible type in read(): expected char, integer, real, array, string");}
            ...
            if (this->args != nullptr)
                for (auto &arg : args->getChildren())
                {
                    ...
                    if (is_ptr_of<LeftExprNode>(arg))
                        ...
                        {eout4();throw CodegenException("Argument in read() must be identifier or array/record reference");}
                    std::vector<llvm::Value*> func_args;
                    ...
                    else if (ptr->getType()->getPointerElementType()->isArrayTy() && llvm::cast<llvm::ArrayType>(ptr->getType()->getPointerElementType())->getElementType()->isIntegerTy(8))
                    {
                        ...
                        {eout4();throw CodegenException("Incompatible type in read(): expected char, integer, real, string");}
        ...
        else if (name == SysFunc::Concat)
        {
            ...
            for (auto &arg : this->args->getChildren()) {
                ...
                else if (value->getType()->isArrayTy()) // String
                {
                    auto *a = llvm::cast<llvm::ArrayType>(x);
                    if (!a->getElementType()->isIntegerTy(8))
                        {eout4();throw CodegenException("Cannot concat a non-char array");}
                    format += "%s";
                    llvm::Value *valuePtr;
                    if (is_ptr_of<LeftExprNode>(arg))
                    {
                ...
                else if (value->getType()->isPointerTy()) // String
                {
                    ...
                    {eout4();throw CodegenException("Incompatible type in concat(): expected char, integer, real, array, string");   }     
            }
        ...
        else if (name == SysFunc::Length)
        {
            ...
            if (args->getChildren().size() != 1)
                {eout4();throw CodegenException("Wrong number of arguments in length(): expected 1");}
            ...
            else if (ty->isPointerTy())
            {
                ...
                {
                    eout4();throw CodegenException("Incompatible type in length(): expected string");
                }
            }
            else
        ...
        else if (name == SysFunc::Abs)
        {
            ...
            if (args->getChildren().size() != 1)
                {eout4();throw CodegenException("Wrong number of arguments in abs(): expected 1");}
            ...
            if (value->getType()->isIntegerTy(32))
                ...
                {eout4();throw CodegenException("Incompatible type in abs(): expected integer, real");}
        }
        else if (name == SysFunc::Val)
        {
            context.log() << "\tSysfunc VAL" << std::endl;
            if (args->getChildren().size() != 1)
                {eout4();throw CodegenException("Wrong number of arguments in val(): expected 1");}
            auto arg = args->getChildren().front();
            auto *value = arg->codegen(context);
            auto *ty = value->getType();
            llvm::Value *zero = llvm::ConstantInt::getSigned(context.getBuilder().getInt32Ty(), 0);
            ...
            else if (ty->isPointerTy())
            {
                ...
                else if (ty->getPointerElementType()->isArrayTy())
                {
                    ...
                    {eout4();throw CodegenException("Incompatible type in val(): expected string");}
            }
            else
                {eout4();throw CodegenException("Incompatible type in val(): expected string");}
        }
        else if (name == SysFunc::Str)
        {
            ...
            if (args->getChildren().size() != 1)
                {eout4();throw CodegenException("Wrong number of arguments in str(): expected 1");}
            auto arg = args->getChildren().front();
            auto *value = arg->codegen(context);
            auto *ty = value->getType();
            llvm::Value *zero = llvm::ConstantInt::getSigned(context.getBuilder().getInt32Ty(), 0);
            ...
            else if (ty->isDoubleTy())
            {
                ...
                {eout4();throw CodegenException("Incompatible type in str(): expected integer, char, real");}
        }
        else if (name == SysFunc::Abs)
        {
            context.log() << "\tSysfunc ABS" << std::endl;
        ...
        else if (name == SysFunc::Sqrt)
        {
            ...
            if (args->getChildren().size() != 1)
                {eout4();throw CodegenException("Wrong number of arguments in sqrt(): expected 1");}
            auto *value = args->getChildren().front()->codegen(context);
            auto *double_ty = context.getBuilder().getDoubleTy();
            if (value->getType()->isIntegerTy(32))
                value = context.getBuilder().CreateSIToFP(value, double_ty);
            else if (!value->getType()->isDoubleTy())
                {eout4();throw CodegenException("Incompatible type in sqrt(): expected integer, real");}
        ...
        else if (name == SysFunc::Sqr)
        {
            ...
            if (args->getChildren().size() != 1)
                {eout4();throw CodegenException("Wrong number of arguments: sqr()");}
            auto *value = args->getChildren().front()->codegen(context);
            if (value->getType()->isIntegerTy(32))      
                ...
                {eout4();throw CodegenException("Incompatible type in sqr(): expected char");}
        }
        else if (name == SysFunc::Chr)
        {
            context.log() << "\tSysfunc CHR" << std::endl;
            if (args->getChildren().size() != 1)
                {eout4();throw CodegenException("Wrong number of arguments in chr(): expected 1");}
            auto *value = args->getChildren().front()->codegen(context);
            if (!value->getType()->isIntegerTy(32))
                {eout4();throw CodegenException("Incompatible type in chr(): expected integer");}
        ...
        else if (name == SysFunc::Ord)
        {
            ...
            if (args->getChildren().size() != 1)
                {eout4();throw CodegenException("Wrong number of arguments in ord(): expected 1");}
            auto *value = args->getChildren().front()->codegen(context);
            if (!value->getType()->isIntegerTy(8))
                {eout4();throw CodegenException("Incompatible type in ord(): expected char");}
        ...
        else if (name == SysFunc::Pred)
        {
            ...
            if (args->getChildren().size() != 1)
                {eout4();throw CodegenException("Wrong number of arguments: pred()");}
            auto *value = args->getChildren().front()->codegen(context);
            if (!value->getType()->isIntegerTy(8))
                {eout4();throw CodegenException("Incompatible type in pred(): expected char");}
        ...
        else if (name == SysFunc::Succ)
        {
            ...
            if (args->getChildren().size() != 1)
                {eout4();throw CodegenException("Wrong number of arguments: succ()");}
            auto *value = args->getChildren().front()->codegen(context);
            if (!value->getType()->isIntegerTy(8))
            {
                {eout4();throw CodegenException("Incompatible type in succ(): expected char");}
            }
    ...
    llvm::Value *RecordRefNode::getPtr(CodegenContext &context)
    {
        ...
        if (recTy == nullptr) {eout4();throw CodegenException(name->getSymbolName() + " is not a record");}
        assert(value->getType()->getPointerElementType()->isStructTy());
	    llvm::Value *idx = recTy->getFieldIdx(field->name, context);
        if (idx == nullptr)
            {eout4();throw CodegenException("'" + field->name + "' is not in record field of " + name->getSymbolName());}
    ...
    llvm::Value *ArrayRefNode::getPtr(CodegenContext &context) 
    {
        ...
        if (ptr_type->isArrayTy())
        {
            ...
            {eout4();throw CodegenException(arr->getSymbolName() + " is not an array");}
